...
export function generateCodeExample(
  ...
  switch (lang) {
    case CodeExampleLang.Curl: {
      let cmd = `curl -X ${request.method} "https://api.signof.io${request.url}"`
      if (parsedHeaders) {
    ...
    case CodeExampleLang.JavaScript: {
      ...
      return `const response = await fetch('https://api.signof.io${request.url}', {\n${opts.join('\n')}\n});\n\nconst data = await response.json();\nconsole.log(data);`
    }
